...
7. **Dashboard** 메뉴는 누구나 헤더/하단 내비게이션에서 볼 수 있습니다. 기본은 최근 7일(KST)이며, 14일·30일·이번 달·직접 선택 기간을 고를 수 있습니다. **선택한 기간**의 접속·글·감성·키워드와 **전체 누적** 게시판 현황을 구분해 보여 줍니다. 현재 접속은 기간과 무관합니다. 이메일은 표시하지 않습니다.
...
| GET | `/api/dashboard`, `/api/admin/dashboard` | 없음 | Dashboard. `insights`/`periodBoard`는 선택 기간, `insightsAll`/`board`는 전체 누적. `preset=7\|14\|30\|month` 또는 `from`/`to`(YYYY-MM-DD, 최대 90일) |
